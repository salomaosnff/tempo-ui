...
export const CursorPagerSymbol = Symbol("CursorPager");

export interface CursorPager<T> extends Pager<T> {
  [CursorPagerSymbol]: true;
  /**
   * Current cursor
   */
...
export function useCursorPager<T>(
  request: (params: CursorPagerParams) => CursorPagerResponse<T> | Promise<CursorPagerResponse<T>>,
...
/**
 * Check if a value is a CursorPager
 */
export function isCursorPager<T = any>(val: any): val is CursorPager<T> {
  return !!val && typeof val === "object" && !!val[CursorPagerSymbol];
}
