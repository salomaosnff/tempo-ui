...
export const OffsetPagerSymbol = Symbol("OffsetPager");

export interface OffsetPager<T> extends Pager<T> {
  [OffsetPagerSymbol]: true;
  /** Current offset */
  offset: Readonly<Ref<number>>;
}

export function useOffsetPager<T>(
  request: (params: OffsetPagerParams) => OffsetPagerResponse<T> | Promise<OffsetPagerResponse<T>>,
  options: UseOffsetPagerOptions = {},
): OffsetPager<T> {
...
/**
 * Check if a value is an OffsetPager
 */
export function isOffsetPager<T = any>(val: any): val is OffsetPager<T> {
  return !!val && typeof val === "object" && !!val[OffsetPagerSymbol];
}
