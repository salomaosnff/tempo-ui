...
import { PagerState, type Pager, isPager, PagerSymbol } from "./pager";
...
export const PagePagerSymbol = Symbol("PagePager");

export interface PagePager<T> extends Pager<T> {
  [PagePagerSymbol]: true;
  /** Current page */
  page: Readonly<Ref<number>>;
}

export function usePagePager<T>(
  request: (params: PagePagerParams) => PagePagerResponse<T> | Promise<PagePagerResponse<T>>,
  options: UsePagePagerOptions = {},
): PagePager<T> {
  ...
  async function next() {
    ...
    const newItems = await send({ ...lastParams.value, page: page.value });
...
/**
 * Check if a value is a PagePager
 */
export function isPagePager<T = any>(val: any): val is PagePager<T> {
  return !!val && typeof val === "object" && !!val[PagePagerSymbol];
}
