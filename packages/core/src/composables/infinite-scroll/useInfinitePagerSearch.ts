...
import {
  useInfinitePager,
  type InfinitePagerScrollElement,
  type InfinitePagerScrollOptions,
} from "./useInfinitePager";
...
export function useInfinitePagerSearch<T>(
  pager: Pager<T>,
  element: InfinitePagerScrollElement = document,
  options: InfinitePagerSearchOptions = {},
) {
  ...
  const infinitePager = useInfinitePager(pager, element, {
    ...infinitePagerOptions,
    immediate: false,
  });
